...
[x] Paso 2. Descomponer PosViewModel: separar catálogo, carrito, pedido actual, adjuntos y workflow offline.
	Completado: extraídos PosDraftService y PosOrderStateService; refresco de carrito/mesa centralizado; catálogo delegado a PosCatalogService; adjuntos delegados a PosMediaService; navegación desacoplada de Shell.Current mediante INavigationService.GoToAsync inyectado en el constructor.
[x] Paso 3. Descomponer PagoViewModel: separar carga de cuenta, validación de cobro, flujo de pago y flujo de comprobante.
	Completado: extraídos PagoComprobanteService, PagoValidationService y PagoWorkflowService; navegación desacoplada de Shell mediante INavigationService; eliminado partial vacío OnCuentaChanged; eliminada la llamada redundante a RefrescarEstadoCobro en OnSaldoPendienteActualChanged (ya cubierta por [NotifyPropertyChangedFor] en la propiedad); eliminado OnUltimoPagoChanged y movidas sus notificaciones a [NotifyPropertyChangedFor(Saldo/PagadoCompleto/PuedeSubirComprobante)] y [NotifyCanExecuteChangedFor] en la declaración de ultimoPago.

[x] Paso 3b. Descomponer HistorialCuentasViewModel: separar filtro, estadísticas y operaciones de cliente/cobro.
...
[x] Paso 5. Reemplazar ScrollView externos en pantallas con CollectionView y restaurar virtualización real.
...
[x] Paso 7. Simplificar ApiService y decidir arquitectura de servicios: fachada única o servicios por dominio inyectables.
